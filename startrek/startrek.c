...
void intro(void) {
    ...
    if (sTemp[0] == 'y' || sTemp[0] == 'Y') showfile("startrek.doc");
...
void new_quadrant(void) {
    ...
    if (k3 > 0) {
        ...
        if (s < 200) printf("Shields Dangerously Low\n");
    }
...
void short_range_scan(void) {
    ...
    if (e < e0 * .1) strcpy(sC, "YELLOW");

    if (k3 > 0) strcpy(sC, "*RED*");

    /* @@@ need to clear the docked flag here */
    d0 = 0;

    /* @@@ for (i = s1 - 1; i <= s1 + 1; i++) */
    for (i = (int)(s1 - 1); i <= (int)(s1 + 1); i++) /* @@@ for (j = s2 - 1; j <= s2 + 1; j++) */
    ...
    printf("------------------------\n");
    for (i = 0; i < 8; i++) {
        for (j = 0; j < 24; j++) putchar(sQ[i * 24 + j]);

        if (i == 0) printf("    Stardate            %d\n", (int)t);
        if (i == 1) printf("    Condition           %s\n", sC);
        if (i == 2) printf("    Quadrant            %d, %d\n", q1, q2);
        if (i == 3) /* @@@ printf("    Sector              %d, %d\n", cint(s1), cint(s2)); */
            printf("    Sector              %d, %d\n", (int)s1, (int)s2);
        if (i == 4) printf("    Photon Torpedoes    %d\n", p);
        if (i == 5) printf("    Total Energy        %d\n", e + s);
        if (i == 6) printf("    Shields             %d\n", s);
        if (i == 7) printf("    Klingons Remaining  %d\n", k9);
    }
    printf("------------------------\n\n");
...
void phaser_control(void) {
    ...
    if (d[8] < 0.0) /* @@@ printf("Computer failure happers accuracy.\n"); */
    ...
    if (d[8] < 0.0) /* @@@ iEnergy = iEnergy * rnd(); */
...
void damage_control(void) {
    ...
    for (r1 = 1; r1 <= 8; r1++) {
        ...
        for (i = 1; i < 25 - (int)strlen(sG2); i++) printf(" ");
        /* @@@ printf("%4.1f\n", d[r1]); */
        printf("%4.2f\n", d[r1]);
    }
...
void galaxy_map(void) {
    ...
    for (i = 1; i <= 8; i++) {
        ...
        for (j = 0; j < j0; j++) printf(" ");

        printf(sG2);

        for (j = 0; j < j0; j++) printf(" ");

        if (!(strlen(sG2) % 2)) printf(" ");
...
void end_of_game(void) {
    ...
    if (b9 > 0) {
        ...
        if (!strncmp(sTemp, "aye", 3)) new_game();
    }
...
void get_device_name(void) {
    static char *device_name[] = {
        "", "Warp Engines", "Short Range Sensors", "Long Range Sensors", "Phaser Control", "Photon Tubes", "Damage Control", "Sheild Control", "Library-Computer"};
...
void quadrant_name(void) {
    static char *quad_name[] = {"",       "Antares", "Rigel",   "Procyon",    "Vega",      "Canopus", "Altair",   "Sagittarius", "Pollux",
                                "Sirius", "Deneb",   "Capella", "Betelgeuse", "Aldebaran", "Regulus", "Arcturus", "Spica"};

    static char *sect_name[] = {"", " I", " II", " III", " IV"};

    if (z4 < 1 || z4 > 8 || z5 < 1 || z5 > 8) strcpy(sG2, "Unknown");
...
int function_r(void) { return (get_rand(8)); }
...
void mid_str(char *a, char *b, int x, int y) {
    ...
    while (x < y && x <= (int)strlen(b)) *a++ = *(b + x++);
...
int get_rand(int iSpread) { return ((rand() % iSpread) + 1); }
